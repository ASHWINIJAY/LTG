...
namespace LTG
{
    public partial class DispatchedDetailReport : System.Web.UI.Page
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            try
            {
                if (!IsPostBack)
                {
                    lblHeader.Text = "Despatched Report - For Date Range Selected " + Session["frmDt"].ToString() + " To " + Session["toDt"].ToString() + " - " + Session["CustomerName"].ToString() + "(" + Session["CustomerCode"].ToString() + ")"; ;
        ...
        private void bindInvoice()
        {
            ...
            {
                con.Open();
                string qry = "Select * from vw_DispatchedReport where DateTimeofScan between '" + Session["frmDt"].ToString() + "' and '" + Session["toDt"].ToString() + "'";

                if (Session["CustomerCode"].ToString() != "All")
                    qry += " and CustomerCode='" + Session["CustomerCode"].ToString() + "'";
        ...
        private void ExportDivContentToExcel1()
        {
            ...
            using (ExcelPackage excel = new ExcelPackage())
            {
                ...
                worksheet.Cells[10, 1].Style.Font.Size = 13;
                worksheet.Cells[10, 1].Style.HorizontalAlignment = OfficeOpenXml.Style.ExcelHorizontalAlignment.Center;
                worksheet.Cells[10, 1, 10, 5].Merge = true;

                var border1 = worksheet.Cells[10, 1, 10, 5].Style.Border;
                ...
                worksheet.Cells[10, 1, 10, 5].Style.Fill.PatternType = ExcelFillStyle.Solid;
                worksheet.Cells[10, 1, 10, 5].Style.Fill.BackgroundColor.SetColor(System.Drawing.ColorTranslator.FromHtml("#FFFF00"));

                worksheet.Cells[10, 6].Value = hdnQty.Value;
                ...
                var border3 = worksheet.Cells[10, 6].Style.Border;
                border3.Top.Style = ExcelBorderStyle.Thin;
                border3.Left.Style = ExcelBorderStyle.Thin;
                border3.Right.Style = ExcelBorderStyle.Thin;
                border3.Bottom.Style = ExcelBorderStyle.Thin;
                worksheet.Cells[10, 6].Style.Fill.PatternType = ExcelFillStyle.Solid;
                worksheet.Cells[10, 6].Style.Fill.BackgroundColor.SetColor(System.Drawing.ColorTranslator.FromHtml("#FFFF00"));
                int i = 0;
                ...
                Response.AddHeader("content-disposition", "attachment; filename=StockOnhandReport.xlsx");
        ...
        protected void grdDetails_RowCreated(object sender, GridViewRowEventArgs e)
        {
            if (e.Row.RowType == DataControlRowType.Header)
            {
                ...
                TableCell mergedCell = CreateSumCell("Total");
                mergedCell.ColumnSpan = 5;
                mergedCell.HorizontalAlign = HorizontalAlign.Center;
                // Merging the first three cells
        ...
        protected void grdDetails_DataBound(object sender, EventArgs e)
        {
            if (grdDetails.Rows.Count > 0)
            {
                ...
                {
                    if (row.Cells[5].Text != "&nbsp;") // Cell index 8 + 1
                        storeQty += Convert.ToInt32(row.Cells[5].Text);
                ...
                sumRow.Cells[1].Text = storeQty.ToString();           // Cell index 8 + 1
                hdnQty.Value = storeQty.ToString();
