...
namespace LTG
{
    public partial class StockTakeConfirm : System.Web.UI.Page
    {
        ...
        private bool ValidateStockMaster()
        {
            ...
            string validationQuery = "SELECT COUNT(*) FROM StocktakeCounterInstruction WHERE Completed = 0";
        ...
        private void loadGrid(bool variance)
        {
            ...
            SqlCommand cmd;
            con.Open();
            string qry = "select QtyOnHand-Isnull(ManagerQty,Qty) as variance,CustomerCode,Isnull(ManagerQty,Qty) as FinalQty,ContainerId,Bin,HU,QtyOnHand,Qty,ManagerQty,AllocatedTo,STN from StocktakeCounterInstruction Where Completed=1 and StockTakeCompleted is null ";
            if (txtStock.Text != "")
                qry += " and HU like '%" + txtStock.Text + "%'";
            if(variance)
            {
                qry += " and QtyOnHand-Isnull(ManagerQty,Qty)>0";
            }
        ...
        private string GenerateGRN()
        {
            ...
            {
                con.Open();
                string qry = "select count(distinct(RefNo))+1 FROM StockTakeHistory";
        ...
        private void save()
        {
            ...
            string query4 = "Update Inbound set IsStockAdj=1 where HU=@HU;Update WarehouseProcess set IsStockAdj=1 where HU=@HU;";
            ...
            using (SqlConnection conn = new SqlConnection(connectionString))
            {
                ...
                foreach (GridViewRow row in grdBin.Rows)
                {
                    if (row.RowType == DataControlRowType.DataRow)
                    {
                        ...
                        string stn = row.Cells[5].Text;
                        string qty = row.Cells[6].Text;
                        string cqty = row.Cells[7].Text;
                        string mqty = row.Cells[8].Text;
                        string variance = row.Cells[9].Text;
                        string fqty = row.Cells[10].Text;
                        ...
                        using (SqlCommand cmd = new SqlCommand(query, conn))
                        {
                            ...
                            cmd.Parameters.AddWithValue("@FQty", fqty);
                        ...
                        //    cmd.Parameters.AddWithValue("@Bin", bin);
                        //    cmd.Parameters.AddWithValue("@HU", hu);
                        //    cmd.Parameters.AddWithValue("@Qty", fqty);
            ...
            loadGrid(false);
            string query3 = "Update StockTakeHistory set RefNo='" + RefNo + "',StockTakeCompleted=1 where RefNo is null;Delete From StockMaster;Update StocktakeCounterInstruction set StockTakeCompleted=1;";
            string query5 = "Update Inbound set StockTakeCompleted=1 where HU=@HU;Update WarehouseProcess set StockTakeCompleted=1 where HU=@HU;";
           ...
           // ClientScript.RegisterStartupScript(this.GetType(), "alert", "alert('Stock Take Process Completed Successfully!');", true);
           // var id = hdnBin.Value;
        ...
        public void CallExecutable(string id)
        {
            ...
            string exePath = @"C:\LTG\LTGStockConfirmReport\LTGStkConfirm.exe";
