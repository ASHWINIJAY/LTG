...
namespace LTG
{
    public partial class UOPMaintain : System.Web.UI.Page
    {
        ...
        private void bindCustomer()
        {
            ...
            using (SqlConnection con = new SqlConnection(constr))
            {
                ...
                if (txtSearchContainer.Text != "")
                    qry += " where UOP like '%" + txtSearchContainer.Text + "%'";
        ...
        protected void grdCustomer_RowCommand(object sender, GridViewCommandEventArgs e)
        {
            ...
            if (e.CommandName == "SelectImage")
                Response.Redirect("UOPCreate.aspx?Code=" + customerCode);
            else
            {
                if (checkCustomer(customerCode))
                {
                    Page.ClientScript.RegisterStartupScript(this.GetType(), "showalert", "alert('You cannot delete as this UOP has already been used and has history');", true);
        ...
        private bool checkCustomer(string cusCode)
        {
            ...
            {
                con.Open();
                string qry = "Select * from Inbound where UOP='" + cusCode + "'";
                SqlCommand cmd1 = new SqlCommand(qry, con);
                using (SqlDataAdapter da = new SqlDataAdapter(cmd1))
                {
                    DataTable dt = new DataTable();
        ...
        {
            grdCustomer.Columns[2].Visible = false;
            ...
            using (ExcelPackage excel = new ExcelPackage())
            {
                ...
                // Add welcome line
                worksheet.Cells[1, 2].Value = "List of UOPs";
                ...
                Response.AddHeader("content-disposition", "attachment; filename=UOPs.xlsx");
                grdCustomer.Columns[2].Visible = true;
        ...
        private void DeleteCustomer(string cusCode)
        {
            ...
            {
                con.Open();
                string qry = "delete  from UOPMaster where UOP='" + cusCode + "'";
                SqlCommand cmd1 = new SqlCommand(qry, con);
                ...
                qry = "Insert into AuditLogs([Table],Field,Custom,ModifiedByUserId,ModifiedByUserName,ModifiedByDate)values('UOPMaster','Delete','Delete the UOP: " + cusCode + " ','" + hdLoginId.Value + "','" + userName + "',getdate())";
                cmd1 = new SqlCommand(qry, con);
                cmd1.ExecuteNonQuery();
                Page.ClientScript.RegisterStartupScript(this.GetType(), "showalert", "alert('UOP deleted successfully.');", true);
