...
namespace LTG
{
    public partial class ReverseWarehouse : System.Web.UI.Page
    {
        ...
        private void bindCustomer()
        {
            ...
            {
                con.Open();
                string qry = "Select * from ReturnReason where ReturnType='Binned Return'";
        ...
        private void GenerateGRN()
        {
            ...
            {
                con.Open();
                string qry = "select count(distinct(RBIN))+1 FROM ReverseWarehouse";
                SqlCommand cmd1 = new SqlCommand(qry, con);
                using (SqlDataAdapter da = new SqlDataAdapter(cmd1))
                {
                    DataTable dt = new DataTable();
                    ...
                    if (dt.Rows.Count > 0)
                    {
                        ...
                        txtGRN.Text = "RBIN-" + dtMonth + "-" + output;
        ...
        private bool checkGDN()
        {
            ...
            {
                con.Open();
                string qry = "Select * from Warehouseprocess where BinningNumber='" + txtGDN.Text+ "' and HU not in(select HU from pickingprocess where isnull(Returned,0)=0) and isnull(BinReturned,0)=0";
                SqlCommand cmd1 = new SqlCommand(qry, con);
                using (SqlDataAdapter da = new SqlDataAdapter(cmd1))
                {
                    DataTable dt = new DataTable();
        ...
        protected void FillGrid()
        {
            ...
            {
                con.Open();
                string qry = "SELECT  distinct BinningNumber,Cast(ScannedInTime as Date) as OutboundDate  FROM WarehouseProcess where HU not in(select HU from Pickingprocess where isnull(Returned,0)=0) and isnull(BinReturned,0)=0";
                if (txtSearchContainer.Text != "")
                    qry += " and BinningNumber like '%" + txtSearchContainer.Text + "%'";
                qry += " order by Cast(ScannedInTime as Date) desc";
                SqlCommand cmd1 = new SqlCommand(qry, con);
                using (SqlDataAdapter da = new SqlDataAdapter(cmd1))
                {
                    DataTable dt = new DataTable();
        ...
        protected void FillHUGrid(string GDN)
        {
            ...
            {
                con.Open();
                string qry = "SELECT  distinct HU  FROM Warehouseprocess Where isnull(BinReturned,0)<>1 and HU not in(select HU from PickingProcess where isnull(Returned,0)=0) and BinningNumber='" + GDN + "'";
                if (txtSearchHU.Text != "")
        ...
        protected void btnSaveHU_Click(object sender, EventArgs e)
        {
            ...
            if (selectedValues.Count > 0)
            {
               // string idList = string.Join(",", selectedValues); // "1,2,3"
                ...
                string query = "SELECT distinct HU,BinningNumber FROM WarehouseProcess WHERE  HU IN (" + result + ")";
                string constr = ConfigurationManager.ConnectionStrings["LTGConn"].ConnectionString;
        ...
        private void save()
        {
            ...
            using (SqlConnection con = new SqlConnection(constr))
            {
                ...
                foreach (GridViewRow row in GridView2.Rows)
                {
                    ...
                    string qry = "Select BinningNumber,CustomerName,CustomerCode,HU,QtyIn,UnitStorageCost,UnitStorageCost,ScannedInTime,Bin from WarehouseProcess Where Isnull(BinReturned,0)=0 and HU='" + Hu + "'";
                    SqlCommand cmd1 = new SqlCommand(qry, con);
                    ...
                    if (dt.Rows.Count > 0)
                    {
                        ...
                        qry = "Update WarehouseProcess set BinReturned=1 where HU='" + Hu + "'";
                        cmd1 = new SqlCommand(qry, con);
cmd1.ExecuteNonQuery();
                        qry = "Insert into ReverseWarehouse(ReasonCode,Reason,RBIN,BinningNumber,CustomerName,CustomerCode,HU,Qty,UnitStorageCost,TotalStorageCost,Loginname,DateTimeofScan,ReturnDateTimeofScan,CreatedBy,CreatedDate,Bin)values('" + ddlReason.SelectedValue + "','" + ddlReason.SelectedItem.Text +"','" + txtGRN.Text + "','" + dt.Rows[0][0].ToString() + "','" + dt.Rows[0][1].ToString() + "','" + dt.Rows[0][2].ToString() + "','" + dt.Rows[0][3].ToString() + "',1,'" + unitCost + "'," + unitCost + ",'" + userid + "','" + dt.Rows[0][7].ToString() + "','" + txtDate.Text + "','" + userName + "',getdate(),'" + dt.Rows[0][8].ToString() + "')";
                        cmd1 = new SqlCommand(qry, con);
                        cmd1.ExecuteNonQuery();
