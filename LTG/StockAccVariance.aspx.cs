...
namespace LTG
{
    public partial class StockAccVariance : System.Web.UI.Page
    {
        ...
        private void ExportDivContentToExcel1()
        {
            ...
            using (ExcelPackage excel = new ExcelPackage())
            {
                ...
                // Add welcome line
                worksheet.Cells[1, 2].Value = "Stock Accuracy Variance Report";
        ...
        {
            if (hdnExportExcel.Value == "0")
            {
                if (e.Row.RowType == DataControlRowType.DataRow)
                {
                    // Assuming BinMatch column is part of your datasource
                    string binMatch = DataBinder.Eval(e.Row.DataItem, "Variance").ToString();

                    if (binMatch == "Yes")  // Change background for "Yes"
                    {
                        e.Row.Attributes["style"] = "background-color: lightpink !important;";

                        e.Row.BackColor = System.Drawing.Color.LightPink;
                    }
                    else if (binMatch == "No") // Optional: highlight mismatches differently
                    {
        ...
        private void loadGrid()
        {
            ...
            SqlCommand cmd;
            con.Open();
            string qry = "select CASE WHEN ActualBin <> Isnull(ManagerCountedBin,CountedBin) THEN 'Yes'  ELSE 'No'   END AS Variance ,isnull(ActualBin,'-') as ActualBin ,isnull(CountedBin,'-') as CountedBin,isnull(ManagerCountedBin,'-') as ManagerCountedBin,HU,isnull(CountedBy,'-') as CountedBy,isnull(MCountedBy,'NA') as MCountedBy from BinAccuracyCount Where StockTakeCompleted is null";
            if (txtStock.Text != "")
