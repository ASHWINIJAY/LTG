...
namespace LTG
{
    public partial class TransportDetailReport : System.Web.UI.Page
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            try
            {
                if (!IsPostBack)
                {
                    lblHeader.Text = "Transport Report - For Date Range Selected " + Session["frmDt"].ToString() + " To " + Session["toDt"].ToString() + " - " + Session["CustomerName"].ToString() + "(" + Session["CustomerCode"].ToString() + ")";
                    bindInvoice();
                }
            }
            catch(Exception ex)
        ...
        private void bindInvoice()
        {
            ...
            {
                con.Open();
                using (SqlCommand cmd = new SqlCommand("Sp_TransportReport", con))
                {
                    ...
                    adapter.Fill(ds);
                    ds.Tables[0].Columns.Add("TransportFee");
                    SqlCommand cmd1;
                    //foreach (DataRow dr in ds.Tables[0].Rows)
                    //{
                    //    cmd1 = new SqlCommand("Select Fee from FeeMaster where FeeType='Transport' and CustomerCode='" + dr["CustomerCode"].ToString() + "'", con);
                    //    dr["TransportFee"]=cmd1.ExecuteScalar();
                    //}
                    if (ds == null)
                    {
                    ...
                    {
                        throw new Exception("No tables found in dataset");
                    }

                    if (!ds.Tables[0].Columns.Contains("TransportFee"))
                    {
                        ds.Tables[0].Columns.Add("TransportFee", typeof(decimal));
                    }

                    if (ds.Tables[0].Rows.Count == 0)
                    {
                        // No rows → nothing to loop
                        return;
                    }
                    string pre = "";
                    string curr = "";
                    ...
                    try
                    {
                        for (int i = ds.Tables[0].Rows.Count - 1; i >= 0; i--)
                        {
                            ...
                            // Delete duplicate GRN rows
                            if (pre == dr["GRN"].ToString())
                            {
                                ds.Tables[0].Rows.Remove(dr);
                                continue;   // Skip remaining processing for deleted rows
                            }

                            using (cmd1 = new SqlCommand(
                                "SELECT Fee FROM FeeMaster WHERE FeeType = @FeeType order by FeeId Desc", con))
                            {
                                cmd1.Parameters.AddWithValue("@FeeType", "Transport");

                                object result = cmd1.ExecuteScalar();
                                dr["TransportFee"] = (result != null) ? result : 0;

                                pre = dr["GRN"].ToString();   // update previous GRN
                            }
                        }
                    }
                    catch (Exception e)
        ...
        private void ExportDivContentToExcel1()
        {
            ...
            using (ExcelPackage excel = new ExcelPackage())
            {
                ...
                foreach (string row in rows)
                {
                    ...
                    if (i > 0)
                    {
                        ...
                        foreach (string cell in cells)
                        {
                            ...
                            if (colNumber == 14 || colNumber == 8 || colNumber == 9 || colNumber == 10)
                            {
                            ...
                            if (colNumber == 17 || colNumber == 18 || colNumber == 20 || colNumber == 19)
                            {
                                excelCell.Style.Fill.PatternType = ExcelFillStyle.Solid;
                ...
                Response.AddHeader("content-disposition", "attachment; filename=Transport.xlsx");
        ...
        protected void grdDetails_DataBound(object sender, EventArgs e)
        {
            if (grdDetails.Rows.Count > 0)
            {
                ...
                foreach (GridViewRow row in grdDetails.Rows)
                {
                    ...
                    if (row.Cells[8].Text != "&nbsp;")
                        qtyIn += Convert.ToDouble(row.Cells[8].Text);
                    if (row.Cells[10].Text != "&nbsp;")
                        totQtyInRate += Convert.ToDouble(row.Cells[10].Text);

                }

                GridViewRow sumRow = grdDetails.Controls[0].Controls[0] as GridViewRow;

                sumRow.Cells[8].Text = qtyIn.ToString();
                sumRow.Cells[10].Text = totQtyInRate.ToString("F2");
