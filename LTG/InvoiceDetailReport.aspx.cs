...
namespace LTG
{
    public partial class InvoiceDetailReport : System.Web.UI.Page
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            try
            {
                if (!IsPostBack)
                {
                    lblHeader.Text = "Detailed Movement Report - HU- For Date Range Selected " + Session["frmDt"].ToString() + " To " + Session["toDt"].ToString() + " - " + Session["CustomerName"].ToString()+"(" + Session["CustomerCode"].ToString() + ")";
                    bindInvoice();
                }
            }
            catch { }
        ...
        private void bindInvoice()
        {
            ...
            {
                con.Open();
                using (SqlCommand cmd = new SqlCommand("Sp_DetailedReport", con))
                {
        ...
        private void ExportDivContentToExcel1()
        {
            ...
            using (ExcelPackage excel = new ExcelPackage())
            {
                ...
                worksheet.Cells[10, 14].Style.Font.Size = 14;
                ...
                Response.AddHeader("content-disposition", "attachment; filename=DetailsReport.xlsx");
        ...
        protected void grdDetails_DataBound(object sender, EventArgs e)
        {
            if (grdDetails.Rows.Count > 0)
            {
                ...
                double totDays = 0;
                    ...
                    foreach (GridViewRow row in grdDetails.Rows)
                    {
                        if (row.Cells[9].Text != "&nbsp;") // Cell index 8 + 1
                            storeQty += Convert.ToDouble(row.Cells[9].Text);

                        if (row.Cells[10].Text != "&nbsp;") // Cell index 9 + 1
                            totDays += Convert.ToDouble(row.Cells[10].Text);

                        if (row.Cells[12].Text != "&nbsp;") // Cell index 11 + 1
                            totalStorageCost += Convert.ToDouble(row.Cells[12].Text);

                        if (row.Cells[14].Text != "&nbsp;") // Cell index 13 + 1
                            qtyIn += Convert.ToDouble(row.Cells[14].Text);

                        if (row.Cells[16].Text != "&nbsp;") // Cell index 15 + 1
                            totQtyInRate += Convert.ToDouble(row.Cells[16].Text);

                        if (row.Cells[18].Text != "&nbsp;") // Cell index 17 + 1
                            qtyOut += Convert.ToDouble(row.Cells[18].Text);

                        if (row.Cells[20].Text != "&nbsp;") // Cell index 19 + 1
                            totOutRate += Convert.ToDouble(row.Cells[20].Text);
                    }
                ...
                sumRow.Cells[20].Text = totOutRate.ToString("F2");    // Cell index 19 + 1
