...
namespace LTG
{
    public partial class DeliveryDetailReport : System.Web.UI.Page
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            try
            {
                if (!IsPostBack)
                {
                    lblHeader.Text = "Delivery Report - For Date Range Selected " + Session["frmDt"].ToString() + " To " + Session["toDt"].ToString() + " - " + Session["CustomerName"].ToString()+"(" + Session["CustomerCode"].ToString() + ")";
                    bindInvoice();
                }
            }
            catch { }
        ...
        private void bindInvoice()
        {
            ...
            {
                con.Open();
                using (SqlCommand cmd = new SqlCommand("Sp_DeliveryReport", con))
                {
                    ...
                    adapter.Fill(ds);
                    ds.Tables[0].Columns.Add("DeliveryFee");
                    SqlCommand cmd1;
                    ...
                    if (!ds.Tables[0].Columns.Contains("DeliveryFee"))
                    {
                        ds.Tables[0].Columns.Add("DeliveryFee", typeof(decimal));
                    }
                    ...
                    try
                    {
                        for (int i = ds.Tables[0].Rows.Count - 1; i >= 0; i--)
                        {
                            ...
                            {
                                cmd1.Parameters.AddWithValue("@FeeType", "Delivery");

                                object result = cmd1.ExecuteScalar();
                                dr["DeliveryFee"] = (result != null) ? result : 0;

                                pre = dr["GDN"].ToString();   // update previous GRN
                            }
                        }
                    }
                    catch (Exception e)
        ...
        private void ExportDivContentToExcel1()
        {
            ...
            using (ExcelPackage excel = new ExcelPackage())
            {
                ...
                Response.AddHeader("content-disposition", "attachment; filename=Delivery.xlsx");
