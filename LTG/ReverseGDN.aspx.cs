...
namespace LTG
{
    public partial class ReverseGDN : System.Web.UI.Page
    {
        ...
        private void bindCustomer()
        {
            ...
            {
                con.Open();
                string qry = "Select * from ReturnReason where ReturnType='GDN Return'";
        ...
        private void GenerateGRN()
        {
            ...
            {
                con.Open();
                string qry = "select count(distinct(RGDN))+1 FROM ReverseGDN";
                SqlCommand cmd1 = new SqlCommand(qry, con);
                using (SqlDataAdapter da = new SqlDataAdapter(cmd1))
                {
                    DataTable dt = new DataTable();
                    ...
                    if (dt.Rows.Count > 0)
                    {
                        ...
                        txtGRN.Text = "RGDN-" + dtMonth + "-" + output;
        ...
        private bool checkGDN()
        {
            ...
            {
                con.Open();
                string qry = "Select * from Outbound where GDN='" +txtGDN.Text+ "'";
                SqlCommand cmd1 = new SqlCommand(qry, con);
                using (SqlDataAdapter da = new SqlDataAdapter(cmd1))
                {
                    DataTable dt = new DataTable();
        ...
        protected void FillGrid()
        {
            ...
            {
                con.Open();
                string qry = "SELECT  distinct GDN,Cast(DateTimeofScan as Date) as OutboundDate  FROM Outbound order by Cast(DateTimeofScan as Date) desc";
                if (txtSearchContainer.Text != "")
                    qry += " and GDN like '%" + txtSearchContainer.Text + "%'";
                SqlCommand cmd1 = new SqlCommand(qry, con);
                using (SqlDataAdapter da = new SqlDataAdapter(cmd1))
                {
                    DataTable dt = new DataTable();
        ...
        protected void FillHUGrid(string GDN)
        {
            ...
            {
                con.Open();
                string qry = "SELECT  distinct HU  FROM Outbound Where isnull(Returned,0)<>1 and GDN='" + GDN + "'";
                if (txtSearchHU.Text != "")
        ...
        protected void btnSaveHU_Click(object sender, EventArgs e)
        {
            ...
            if (selectedValues.Count > 0)
            {
               // string idList = string.Join(",", selectedValues); // "1,2,3"
                ...
                string query = "SELECT distinct HU,GDN FROM Outbound WHERE  HU IN (" + result + ")";
                string constr = ConfigurationManager.ConnectionStrings["LTGConn"].ConnectionString;
        ...
        private void save()
        {
            ...
            using (SqlConnection con = new SqlConnection(constr))
            {
                ...
                foreach (GridViewRow row in GridView2.Rows)
                {
                    ...
                    string qry = "Select GDN,CustomerName,CustomerCode,HU,Qty,UnitOutBoundCost,TotalOutBoundCost,DateTimeofScan from Outbound Where HU='" + Hu + "'";
                    SqlCommand cmd1 = new SqlCommand(qry, con);
                    ...
                    if (dt.Rows.Count > 0)
                    {
                        ...
                        if(days<0)
                        {
                            string script = "alert(\"Returned Date Should not be less then GDN Date\");";
                            ScriptManager.RegisterStartupScript(this, GetType(),
                                                  "ServerControlScript", script, true);
                            break;
                        }
                        ...
                        qry = "Update WarehouseProcess set Returned=1,QtyOut=0,QtyOnHand=1,ScannedOutTime=null,ModifiedBy='" + userName + "',ModifiedDate=getdate() where HU='" + Hu + "'";
                        cmd1 = new SqlCommand(qry, con);
                        cmd1.ExecuteNonQuery();
                        qry = "Update Outbound set Returned=1 where HU='" + Hu + "'";
                        cmd1 = new SqlCommand(qry, con);
cmd1.ExecuteNonQuery();
                        qry = "Insert into ReverseGDN(ReasonCode,Reason,RGDN,GDN,CustomerName,CustomerCode,HU,Qty,UnitOutBoundCost,TotalOutBoundCost,Loginname,DateTimeofScan,ReturnDateTimeofScan,CreatedBy,CreatedDate)values('" + ddlReason.SelectedValue + "','" + ddlReason.SelectedItem.Text +"','" + txtGRN.Text + "','" + dt.Rows[0][0].ToString() + "','" + dt.Rows[0][1].ToString() + "','" + dt.Rows[0][2].ToString() + "','" + dt.Rows[0][3].ToString() + "',1,'" + unitCost + "'," + unitCost + ",'" + userid + "','" + dt.Rows[0][7].ToString() + "','" + txtDate.Text + "','" + userName + "',getdate())";
                        cmd1 = new SqlCommand(qry, con);
                        cmd1.ExecuteNonQuery();
