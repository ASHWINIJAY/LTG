...
namespace LTG
{
    public partial class BinnedDetailReport : System.Web.UI.Page
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            try
            {
                if (!IsPostBack)
                {
                    lblHeader.Text = "Binned Detailed Report - For Date Range Selected " + Session["frmDt"].ToString() + " To " + Session["toDt"].ToString() + " - " + Session["CustomerName"].ToString()+"(" + Session["CustomerCode"].ToString() + ")";
                    bindInvoice();
                }
            }
            catch { }
        ...
        private void bindInvoice()
        {
            ...
            {
                con.Open();
                using (SqlCommand cmd = new SqlCommand("Sp_WarehouseReport", con))
                {
        ...
        private void ExportDivContentToExcel1()
        {
            ...
            using (ExcelPackage excel = new ExcelPackage())
            {
                ...
                foreach (string row in rows)
                {
                    ...
                    if (i > 0)
                    {
                        ...
                        foreach (string cell in cells)
                        {
                            ...
                            if (colNumber == 14 || colNumber == 16 || colNumber == 15 || colNumber == 13)
                            {
                                excelCell.Style.Fill.PatternType = ExcelFillStyle.Solid;
                ...
                Response.AddHeader("content-disposition", "attachment; filename=BinnedReport.xlsx");
        ...
        protected void grdDetails_DataBound(object sender, EventArgs e)
        {
            if (grdDetails.Rows.Count > 0)
            {
                ...
                sumRow.Cells[9].Text = totDays.ToString();
                sumRow.Cells[11].Text = totalStorageCost.ToString("F2");
