...
namespace LTG
{
    public partial class RWarehouseDetailReport : System.Web.UI.Page
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            try
            {
                if (!IsPostBack)
                {
                    lblHeader.Text = "Binned Return Report - For Date Range Selected " + Session["frmDt"].ToString() + " To " + Session["toDt"].ToString() + " - " + Session["CustomerName"].ToString() + "(" + Session["CustomerCode"].ToString() + ")";
                    bindInvoice();
                }
            }
            catch { }
        ...
        private void bindInvoice()
        {
            ...
            {
                con.Open();
                using (SqlCommand cmd = new SqlCommand("Sp_RBinnedReport", con))
                {
        ...
        private void ExportDivContentToExcel1()
        {
            ...
            using (ExcelPackage excel = new ExcelPackage())
            {
                ...
                foreach (string row in rows)
                {
                    ...
                    if (i > 0)
                    {
                        ...
                        foreach (string cell in cells)
                        {
                            ...
                            if (colNumber != 2 &&  double.TryParse(cellTrim, out double numericValue))
                            {
                ...
                Response.AddHeader("content-disposition", "attachment; filename=ReverseGDNReport.xlsx");
        ...
        protected void grdDetails_DataBound(object sender, EventArgs e)
        {
            if (grdDetails.Rows.Count > 0)
            {
                ...
                foreach (GridViewRow row in grdDetails.Rows)
                {
                    ...
                    //if (row.Cells[8].Text != "&nbsp;")
                    //    totOutRate += Convert.ToDouble(row.Cells[8].Text);
                ...
                //sumRow.Cells[8].Text = totOutRate.ToString();
            }
        }
    }
}