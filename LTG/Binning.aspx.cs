...
namespace LTG
{
    public partial class Binning : System.Web.UI.Page
    {
        ...
        protected void grdOutBound_RowDataBound(object sender, GridViewRowEventArgs e)
        {
            if (e.Row.RowType == DataControlRowType.DataRow)
            {
                ...
                int qty = Convert.ToInt32(DataBinder.Eval(e.Row.DataItem, "QtyIn"));
                totalQty += qty;
            }
            else if (e.Row.RowType == DataControlRowType.Footer)
            {
                e.Row.Cells[2].Text = "Total Qty Of HU Binned";
        ...
        private void FillData(string id)
        {
            ...
            using (SqlConnection con = new SqlConnection(constr))
            {
                ...
                string qry = "";
                lblSlip.Text = id;
                qry = "select * from WarehouseProcess where BinningNumber='" + lblSlip.Text + "' order by CreatedDate";
