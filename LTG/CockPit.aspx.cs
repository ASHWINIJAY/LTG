...
namespace LTG
{
    public partial class CockPit : System.Web.UI.Page
    {
        protected void grdBin_RowDataBound(object sender, GridViewRowEventArgs e)
        {
            if (e.Row.RowType == DataControlRowType.DataRow)
            {
                ...
                if (ViewState["CheckedBoxes"] is List<string> checkedBoxes && checkedBoxes.Contains(hu))
                {
                    ...
                    if (ViewState["CheckedBoxesBin"] is List<string> checkedBoxes1 && checkedBoxes1.Contains(lblBin.Text))
                    {
                        chkSelect.Checked = true;
                    }
        ...
        private DataTable loadData()
        {
            ...
            SqlCommand cmd;
            con.Open();
            string qry = "select Distinct HU,Bin,CustomerCode from StockMaster where (counter1allocate is null or counter1allocate='')";
        ...
        private void loadBinGrid()
        {
          ...
          //  SaveCheckboxStates();
            ...
            SqlCommand cmd;
            con.Open();
            string qry = "select Distinct Bin,CustomerCode from StockMaster where (counter1allocate is null or counter1allocate='')";
            if (txtBins.Text != "")
        ...
        protected void AllocateStock()
        {
            ...
            string query1 = "UPDATE StocktakeCounterInstruction SET Allocated = 1, AllocatedTo = @UserName, AllocatedBy = @AllocatedBy, AllocatedDate = GETDATE() WHERE HU = @Bin";
